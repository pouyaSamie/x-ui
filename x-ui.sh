...
# check root
[[ $EUID -ne 0 ]] && LOGE "Error: This script must be run as root user!\n" && exit 1
...
confirm_restart() {
    confirm "Whether to restart the panel. If you restart the panel, it will also restart. xray" "y"
    if [[ $? == 0 ]]; then
        restart
    else
        show_menu
...
update() {
    confirm "This function will force the latest version to be reinstalled. No data will be lost. Do you want to continue??" "n"
    if [[ $? != 0 ]]; then
        LOGE "已取消"
        if [[ $# == 0 ]]; then
            before_show_menu
...
reset_config() {
    ...
    echo -e "All panel settings have been reset to default. Please restart the panel now and access the panel using the default ${green}54321${plain} ports."
    confirm_restart
}
...
set_port() {
    ...
    if [[ -z "${port}" ]]; then
        ...
        echo -e "The port is set. Now please restart the panel and use the newly set port. ${green}${port}${plain} 访问面板"
        confirm_restart
    fi
}
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/pouyasamie/x-ui/raw/master/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_enable_status() {
    check_enabled
    if [[ $? == 0 ]]; then
        echo -e "Whether to start automatically at boot: ${green} is ${plain}"
    else
...
————————————————
  ${green}15.${plain} 一One-click installation of bbr (latest kernel)
  ${green}16.${plain} 一Apply for SSL certificate (acme application)
 "
    show_status
